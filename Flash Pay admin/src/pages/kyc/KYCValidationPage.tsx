      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {loading ? (
          ...
          <div className="col-span-full m3-card-elevated p-20 text-center bg-[#F3EDF7]/30 border-dashed border-2">
            <ShieldCheck className="mx-auto text-[#6344B6]/20 mb-6" size={64} />
            <p className="text-[#49454F] font-black uppercase text-[10px] tracking-widest">Aucun dossier trouvé</p>
          </div>
        ) : (
          filteredRequests.map((req) => (
            <div key={req.id} className="m3-card-elevated group hover:border-[#6344B6]/30">
              <div className="flex justify-between items-start mb-8">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 bg-[#EADDFF] text-[#21005D] rounded-[20px] flex items-center justify-center font-black text-xl shadow-sm border border-[#6344B6]/10 group-hover:scale-110 transition-transform">
              ...
              <div className="space-y-4 mb-8">
                ...
                <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-[#49454F]">
                  ...
                  <span className="text-[#6344B6] font-mono">#{req.userId.substring(0, 8).toUpperCase()}</span>
                </div>
              </div>

              <button 
                onClick={() => setSelectedRequest(req)}
                className="w-full m3-btn-tonal !rounded-[20px] group-hover:bg-[#6344B6] group-hover:text-white transition-all shadow-sm"
              >
                  ...
                  return (
                    <div key={docType.key} className="space-y-4">
                      <div className="flex justify-between items-center px-1">
                        ...
                        <span className="text-[9px] font-bold text-[#6344B6] uppercase opacity-40">{docData?.type || 'Image'}</span>
                      </div>
                          ...
                          isPdfDocument(docData.url) ? (
                            <>
                              ...
                              <div className="absolute inset-0 bg-[#1D1B20]/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                 <button onClick={() => handleOpenDocument(docData.url)} className="p-4 bg-white rounded-full shadow-2xl text-[#6344B6] hover:scale-110 transition-transform">
              ...
              <div className="space-y-8">
                <div className="bg-[#F3EDF7] p-8 rounded-[32px] border border-[#E7E0EB] space-y-6">
                   <div className="flex items-center gap-3 border-b border-[#E7E0EB] pb-4">
                     <FileText size={20} className="text-[#6344B6]" />
                   ...
                   <div className="grid grid-cols-2 gap-6">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs font-mono font-black text-[#6344B6]">#{selectedRequest.userId.substring(0, 10).toUpperCase()}</p>
                      </div>
                      <div className="space-y-1">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs font-black text-[#6344B6] uppercase">{selectedRequest.status}</p>
                      </div>
                   </div>
                </div>
                ...
                {selectedRequest.status === 'pending' ? (
                  <div className="space-y-6">
                    <div className="space-y-2">
                       ...
                       <textarea 
                        ...
                        className="w-full bg-[#F3EDF7] border border-[#CAC4D0] rounded-[24px] p-5 text-sm font-medium text-[#1D1B20] focus:ring-4 focus:ring-[#6344B6]/10 transition-all h-32"
                       />
                    </div>
                  ...
                  <div className={`p-8 rounded-[32px] border flex flex-col items-center text-center gap-4 ${
                    selectedRequest.status === 'approved' ? 'bg-[#E8DEF8] border-[#6344B6]/20' : 'bg-[#F9DEDC] border-[#B3261E]/20'
                  }`}>
