    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          <h2 className="text-3xl font-black text-[#1D1B20] tracking-tight flex items-center gap-4">
            <div className="w-12 h-12 bg-[#470B37] text-white rounded-[16px] flex items-center justify-center shadow-lg"><Percent size={24} /></div>
            Gestion des Frais
          </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-6">
          {commissions.length === 0 ? (
            <div className="m3-card-elevated p-20 text-center bg-[#F3EDF7]/30 border-2 border-dashed">
               <Settings className="mx-auto text-[#470B37]/20 mb-6" size={64} />
            ...
            commissions.sort((a, b) => a.transferType.localeCompare(b.transferType)).map(rule => (
              <div key={rule.id} className="m3-card-elevated group relative overflow-hidden transition-all hover:border-[#470B37]/30">
                <div className="flex justify-between items-start">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        {rule.destinationCountry && (
                          <div className="flex items-center gap-1.5 bg-[#F3EDF7] px-3 py-1 rounded-full text-[9px] text-[#470B37] font-black uppercase tracking-widest border border-[#E7E0EB]">
                            <Globe size={12} /> {rule.destinationCountry}
                          </div>
                        )}
                        {rule.destinationOperator && (
                          <div className="flex items-center gap-1.5 bg-[#EADDFF] px-3 py-1 rounded-full text-[9px] text-[#21005D] font-black uppercase tracking-widest border border-[#470B37]/10">
                  ...
                  <div className="text-right">
                    <p className="text-3xl font-black text-[#470B37] tracking-tighter">
                ...
                <div className="absolute top-6 right-8 opacity-0 group-hover:opacity-100 transition-all flex gap-3 translate-x-4 group-hover:translate-x-0">
                  <button 
                    ...
                    className="p-3 bg-white border border-[#E7E0EB] text-[#470B37] hover:bg-[#470B37] hover:text-white rounded-xl shadow-lg transition-all"
                  >
                    <Save size={18} />
                  </button>
                  <button 
        ...
        <div className="space-y-8">
          {isAdding && (
            <div className="m3-card-elevated !bg-[#FEF7FF] border-[#470B37]/20 relative animate-in slide-in-from-right-10 duration-500 shadow-2xl">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <select 
                    ...
                    className="w-full bg-[#F3EDF7] border border-[#CAC4D0] rounded-2xl px-5 py-4 text-sm font-bold text-[#1D1B20] focus:ring-4 focus:ring-[#470B37]/10 transition-all outline-none appearance-none"
                  >
                ...
                <div>
                  ...
                  <div className="flex bg-[#F3EDF7] p-1.5 rounded-full border border-[#E7E0EB] shadow-inner">
                    <button 
                      ...
                      className={`flex-1 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${formData.feeType === 'percentage' ? 'bg-[#470B37] text-white shadow-md' : 'text-[#49454F] hover:bg-white/50'}`}
                    >
                      Pourcentage
                    </button>
                    <button 
                      onClick={() => setFormData({ ...formData, feeType: 'fixed' })}
                      className={`flex-1 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${formData.feeType === 'fixed' ? 'bg-[#470B37] text-white shadow-md' : 'text-[#49454F] hover:bg-white/50'}`}
                    >
                      Somme Fixe
                    </button>
                  </div>
                ...
                <div>
                  ...
                  <div className="relative group">
                    <input 
                      ...
                      className="w-full bg-[#F3EDF7] border border-[#CAC4D0] rounded-2xl px-5 py-4 text-lg font-black text-[#1D1B20] focus:ring-4 focus:ring-[#470B37]/10 transition-all outline-none"
                    />
                    <span className="absolute right-5 top-1/2 -translate-y-1/2 text-[#470B37] font-black text-sm">
          ...
          <div className="m3-card-elevated border-[#E7E0EB] !p-8">
            <h3 className="text-lg font-black text-[#1D1B20] tracking-tight mb-8 flex items-center gap-3">
              <History className="text-[#470B37]" size={20} />
               ...
               {logs.length === 0 ? (
                 ...
                 logs.map(log => (
                   <div key={log.id} className="relative pl-6 border-l-2 border-[#F3EDF7] group/log">
                     <div className="absolute -left-[5px] top-0 w-2 h-2 rounded-full bg-[#470B37] group-hover/log:scale-150 transition-transform"></div>
          ...
          <div className="bg-[#EADDFF] p-6 rounded-[28px] border border-[#470B37]/10 flex gap-5 items-start">
             <div className="p-3 bg-white rounded-2xl shadow-sm text-[#470B37]"><Info size={24} /></div>
