...
const RateCard = ({ rate, onEdit }: { rate: ExchangeRate; onEdit: (rate: ExchangeRate) => void }) => {
  ...
  const getStyle = (currency: string) => {
    ...
    return 'bg-[#F3EDF7] text-[#6344B6]';
  };

  return (
    <div className="m3-card-elevated group hover:border-[#6344B6]/30 transition-all">
      <div className="flex justify-between items-start mb-8">
        ...
        <button 
          onClick={() => onEdit(rate)}
          className="p-3 bg-[#F3EDF7] text-[#6344B6] hover:bg-[#6344B6] hover:text-white rounded-xl transition-all shadow-sm group-hover:scale-110"
        >
          <Edit3 size={18} />
        </button>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex justify-between items-center px-5 py-3 bg-[#EADDFF]/50 rounded-full border border-[#6344B6]/10">
          <span className="text-[#21005D] text-[9px] font-black uppercase tracking-[0.2em]">Marge Opérationnelle</span>
          <span className="text-[#6344B6] font-black text-sm">{(rate.margin * 100).toFixed(1)}%</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          <h2 className="text-3xl font-black text-[#1D1B20] tracking-tight flex items-center gap-4">
            <div className="w-12 h-12 bg-[#6344B6] text-white rounded-[16px] flex items-center justify-center shadow-lg"><ArrowRightLeft size={24} /></div>
            Devises & Systèmes
          </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
           ...
           <div className="col-span-full m3-card-elevated !p-0 overflow-hidden mt-4">
             <div className="p-8 border-b border-[#E7E0EB] flex justify-between items-center">
                <h3 className="text-xl font-black text-[#1D1B20] tracking-tight flex items-center gap-3">
                  <Coins className="text-[#6344B6]" size={20} /> Paires Additionnelles
                </h3>
                    ...
                    {customRates.length === 0 ? (
                      ...
                      customRates.map(crate => (
                        <tr key={crate.id} className="hover:bg-[#F3EDF7]/20 transition-all group">
                          ...
                          <td className="px-8 py-5 text-sm font-mono font-black text-[#6344B6]">{crate.rate.toFixed(2)}</td>
                          ...
                          <td className="px-8 py-5 text-right flex justify-end gap-3">
                             <button onClick={() => handleEditCustomRate(crate)} disabled={!canEdit} className="p-2 text-[#6344B6] hover:bg-[#F3EDF7] hover:text-[#6344B6] transition-all disabled:opacity-30 disabled:cursor-not-allowed rounded-lg"><Edit3 size={16} /></button>
        ...
        <div className="m3-card-elevated !bg-[#6344B6] text-white shadow-2xl">
           ...
           <div className="space-y-8">
                 ...
                 <div className="space-y-3">
                    ...
                    >
                      {availableCurrencies.length === 0 && (
                        <option value={editingPointsCurrency} className="bg-[#6344B6]">{editingPointsCurrency}</option>
                      )}
                      {availableCurrencies.map(cur => (
                        <option key={cur} value={cur} className="bg-[#4A3191] text-white font-black">{cur}</option>
                      ))}
                    </select>
                 </div>
              ...
              <div className="pt-8 border-t border-white/10 space-y-5">
                 ...
                 <div className="flex gap-3">
                    ...
                    <button onClick={addEmail} disabled={!canAdd} className="p-3 bg-white text-[#6344B6] rounded-full shadow-lg hover:scale-110 transition-transform disabled:opacity-40 disabled:cursor-not-allowed"><Plus size={18} /></button>
                 </div>
              ...
              <button onClick={handleSaveSettings} disabled={isSavingSettings} className="w-full py-5 bg-white text-[#6344B6] rounded-[28px] font-black uppercase text-[11px] tracking-widest shadow-2xl hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50">
                ...
                {editingRate ? (
                  <>
                    <div>
                       ...
                       <div className="relative">
                          <TrendingUp className="absolute left-5 top-1/2 -translate-y-1/2 text-[#6344B6]" size={20} />
                    ...
                    <div>
                       ...
                       <div className="relative">
                          ...
                          />
                          <span className="absolute right-6 top-1/2 -translate-y-1/2 text-[#6344B6] font-black">%</span>
                       </div>
