      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <div key={country.id} className="m3-card-elevated group relative overflow-hidden">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[#6344B6] text-[10px] font-black uppercase tracking-widest bg-[#EADDFF] px-2 py-0.5 rounded-md">{(country as any).dialCode || '+???'}</span>
                    <span className="text-[#49454F] text-[10px] font-bold uppercase tracking-widest">• {country.currency}</span>
                  </div>
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => handleEdit(country)} className="p-2.5 bg-[#F3EDF7] text-[#6344B6] rounded-full hover:bg-[#EADDFF] transition-all"><Edit3 size={16} /></button>
            ...
            <div className="space-y-4 mb-8">
              ...
              <div className="h-1.5 bg-[#F3EDF7] rounded-full overflow-hidden">
                <div className="h-full bg-[#6344B6] rounded-full" style={{ width: `${Math.min(100, country.operators.length * 20)}%` }}></div>
              </div>
              <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-[#49454F]">
            ...
            <div className="pt-6 border-t border-[#E7E0EB] flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <button onClick={() => setRestrictionModal({ open: true, country })} className="p-2.5 bg-[#F3EDF7] text-[#6344B6] rounded-full hover:bg-[#EADDFF] transition-all">
                  <Settings2 size={16} />
                </button>
              </div>
            </div>
                  ...
                  {sortedOperators.map((op, idx) => (
                    <tr key={idx} className="hover:bg-[#F3EDF7]/30 transition-all group">
                      ...
                      <td className="px-8 py-5">
                         <div className="flex flex-wrap gap-1.5">
                           {(op as any).prefixes?.map((p: string, i: number) => (
                             <span key={i} className="px-2 py-0.5 bg-[#F3EDF7] text-[#6344B6] rounded text-[9px] font-mono font-bold border border-[#E7E0EB]">{p}</span>
                      ...
                      <td className="px-8 py-5 text-right">
                        <button onClick={() => { setActiveTab('countries'); handleEdit(op.parentCountry); }} className="p-2.5 bg-[#F3EDF7] text-[#6344B6] rounded-xl hover:bg-[#EADDFF] transition-all opacity-0 group-hover:opacity-100"><Edit3 size={16} /></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        {activeTab === 'banks' && sortedBanks.map(bank => (
          <div key={bank.id} className="m3-card-elevated group">
            <div className="flex justify-between items-start mb-8">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 bg-[#EADDFF] rounded-[20px] flex items-center justify-center text-[#21005D] border border-[#6344B6]/10 shadow-sm overflow-hidden">
              ...
              <div className="flex gap-2">
                <button onClick={() => handleEdit(bank)} className="p-2.5 bg-[#F3EDF7] text-[#6344B6] rounded-full hover:bg-[#EADDFF] transition-all"><Edit3 size={16} /></button>
            ...
            <div className="bg-[#F3EDF7] p-5 rounded-[24px] border border-[#E7E0EB] mb-8 group/num relative overflow-hidden">
               ...
               <p className="text-[#1D1B20] font-mono font-black text-lg tracking-tight flex items-center gap-2">
                 <Phone size={14} className="text-[#6344B6]" /> {bank.number}
               </p>
               {bank.holder && <p className="text-[10px] font-bold text-[#6344B6] mt-2 uppercase tracking-wide opacity-70 truncate">{bank.holder}</p>}
            </div>
                ...
                {activeTab === 'countries' ? (
                  <div className="space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <input name="name" required defaultValue={editingItem?.name} placeholder="Cameroun" className="w-full bg-[#F3EDF7] border border-[#CAC4D0] rounded-2xl px-5 py-4 text-sm font-bold text-[#1D1B20] focus:ring-4 focus:ring-[#6344B6]/10 transition-all" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-[#49454F] uppercase tracking-widest ml-1">Code ISO (CM, CI...)</label>
                        <input name="code" required defaultValue={editingItem?.code} placeholder="CM" className="w-full bg-[#F3EDF7] border border-[#CAC4D0] rounded-2xl px-5 py-4 text-sm font-bold text-[#1D1B20] focus:ring-4 focus:ring-[#6344B6]/10 transition-all" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-[#49454F] uppercase tracking-widest ml-1">Devise Locale</label>
                        <input name="currency" required defaultValue={editingItem?.currency} placeholder="XAF" className="w-full bg-[#F3EDF7] border border-[#CAC4D0] rounded-2xl px-5 py-4 text-sm font-bold text-[#1D1B20] focus:ring-4 focus:ring-[#6344B6]/10 transition-all" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-[#49454F] uppercase tracking-widest ml-1">Indicatif (+)</label>
                        <input name="dialCode" required defaultValue={editingItem?.dialCode} placeholder="+237" className="w-full bg-[#F3EDF7] border border-[#CAC4D0] rounded-2xl px-5 py-4 text-sm font-bold text-[#1D1B20] focus:ring-4 focus:ring-[#6344B6]/10 transition-all" />
                      </div>
                    </div>
                    ...
                    <div className="space-y-6 pt-4 border-t border-[#E7E0EB]">
                      <div className="flex justify-between items-center">
                        <h4 className="text-sm font-black text-[#1D1B20] uppercase tracking-widest flex items-center gap-2">
                          <Smartphone size={16} className="text-[#6344B6]" /> Gestion des Opérateurs
                        </h4>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                               ...
                               <div className="col-span-full flex items-center gap-4 bg-white/50 p-3 rounded-2xl border border-dashed border-[#E7E0EB]">
                                  <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center border border-[#E7E0EB]">
                                    {op.logo ? <img src={op.logo} className="w-full h-full object-contain p-1" /> : <Upload size={14} className="text-[#6344B6]/30" />}
                                  </div>
                  ...
                  <div className="space-y-6">
                    <div className="space-y-2">
                      ...
                      <input name="name" required defaultValue={editingItem?.name} placeholder="Sberbank" className="w-full bg-[#F3EDF7] border border-[#CAC4D0] rounded-2xl px-5 py-4 text-sm font-bold text-[#1D1B20] focus:ring-4 focus:ring-[#6344B6]/10 transition-all" />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-[#49454F] uppercase tracking-widest ml-1">Type de Compte</label>
                        <select name="type" defaultValue={editingItem?.type || 'phone'} className="w-full bg-[#F3EDF7] border border-[#CAC4D0] rounded-2xl px-5 py-4 text-sm font-bold text-[#1D1B20] focus:ring-4 focus:ring-[#6344B6]/10 transition-all appearance-none">
                      ...
                      <div className="space-y-2">
                        ...
                        <input name="number" required defaultValue={editingItem?.number} placeholder="+7 900..." className="w-full bg-[#F3EDF7] border border-[#CAC4D0] rounded-2xl px-5 py-4 text-sm font-mono font-bold text-[#1D1B20] focus:ring-4 focus:ring-[#6344B6]/10 transition-all" />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-[#49454F] uppercase tracking-widest ml-1">Titulaire du Compte</label>
                      <input name="holder" defaultValue={editingItem?.holder} placeholder="Nom du détenteur" className="w-full bg-[#F3EDF7] border border-[#CAC4D0] rounded-2xl px-5 py-4 text-sm font-bold text-[#1D1B20] focus:ring-4 focus:ring-[#6344B6]/10 transition-all" />
                    </div>
                    <div className="space-y-2">
        ...
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4 animate-in fade-in duration-300">
          ...
          <div className="relative bg-white w-full max-w-md rounded-[40px] shadow-2xl overflow-hidden p-8 border border-[#E7E0EB]" onClick={e => e.stopPropagation()}>
            <div className="text-center mb-8">
              ...
              <p className="text-[#49454F] text-sm font-medium">Définissez les restrictions pour <span className="text-[#6344B6] font-black">{restrictionModal.country?.name}</span></p>
            </div>

            <div className="space-y-3">
               {[
                 { id: 'full', label: 'Service Complet', desc: 'Double sens (RU ↔ AF)', icon: Zap, color: '#6344B6' },
                 ...
                 <button
                   ...
                   className="w-full flex items-center gap-4 p-5 rounded-[28px] bg-[#F3EDF7] border border-transparent hover:border-[#6344B6]/20 hover:bg-white transition-all text-left shadow-sm group"
                 >
          ...
          <div className="relative bg-white w-full max-w-sm rounded-[40px] shadow-2xl overflow-hidden p-8 border border-[#E7E0EB]" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-[#F3EDF7] p-6 rounded-[28px] border border-[#E7E0EB] mb-8">
               <div className="flex items-center justify-between mb-4">
                  <span className="text-[10px] font-black text-[#6344B6] uppercase tracking-widest">Paire</span>
               ...
               <div className="space-y-2">
                  ...
                  <div className="relative">
                    <input 
                      ...
                      placeholder="0.0000"
                      id="rateInput"
                      className="w-full bg-white border-2 border-[#E7E0EB] rounded-2xl px-5 py-4 text-lg font-mono font-black text-[#1D1B20] focus:border-[#6344B6] outline-none transition-all"
                    />
                    <span className="absolute right-5 top-1/2 -translate-y-1/2 text-[10px] font-black text-[#6344B6] uppercase tracking-widest">{rateModal.to}</span>
                  </div>
               </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button 
                ...
                className="flex-1 bg-[#6344B6] text-white py-4 font-black uppercase text-[10px] tracking-widest rounded-full shadow-lg shadow-[#6344B6]/20 hover:bg-[#5D4696] transition-all"
              >
                Confirmer
              </button>
            </div>
