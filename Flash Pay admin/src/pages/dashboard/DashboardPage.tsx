  ...
  return (
    <div className="m3-card-elevated group relative overflow-hidden">
      ...
      <div className="flex justify-between items-start mb-6">
        <div className="p-4 bg-[#F3EDF7] text-[#470B37] rounded-[20px] group-hover:scale-110 transition-transform duration-500 shadow-sm border border-[#E7E0EB]">
  ...
  return (
    <div className="space-y-10 pb-20 animate-in fade-in duration-700">
        ...
        <div className="flex flex-wrap items-center gap-2 p-1.5 bg-[#F3EDF7] rounded-[24px] border border-[#E7E0EB] shadow-sm">
           {[
             ...
             <button 
              ...
              className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${timeRange === range.id ? 'bg-[#470B37] text-white shadow-md' : 'text-[#49454F] hover:bg-white'}`}
             >
               {range.label}
             </button>
           ))}
              ...
              <AreaChart data={chartData}>
                <defs>
                  <linearGradient id="colorValue" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#470B37" stopOpacity={0.2}/>
                ...
                <Tooltip 
                  ...
                  itemStyle={{ color: '#470B37', fontWeight: 800, fontSize: '14px' }}
                  cursor={{ stroke: '#470B37', strokeWidth: 2, strokeDasharray: '5 5' }}
                />
                <Area type="monotone" dataKey="value" stroke="#470B37" strokeWidth={5} fillOpacity={1} fill="url(#colorValue)" animationDuration={2000} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              ...
              popularRoutes.map((route, i) => (
                <div key={i} className="group">
                  <div className="flex justify-between items-end mb-3 px-1">
                    ...
                    <span className="text-[10px] text-[#470B37] font-black uppercase tracking-widest">{route.count} transferts</span>
                  </div>
                  <div className="h-4 bg-[#F3EDF7] rounded-full overflow-hidden p-0.5 border border-[#E7E0EB]">
                    <div 
                      className="h-full bg-gradient-to-r from-[#470B37] to-[#9B89C9] rounded-full transition-all duration-1000 shadow-lg shadow-[#470B37]/20" 
        ...
        <div className="m3-card-elevated">
          <div className="flex items-center gap-3 mb-10">
            <div className="p-2 bg-[#F3EDF7] text-[#470B37] rounded-lg shadow-sm"><Clock size={18} /></div>
            ...
            {recentTransactions.map((tx) => (
              <div key={tx.id} onClick={() => navigate(`/queue/${tx.id}`)} className="flex items-center gap-4 p-4 hover:bg-[#F3EDF7]/50 rounded-[28px] transition-all border border-transparent hover:border-[#E7E0EB] group cursor-pointer shadow-sm bg-white/50">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[9px] font-black text-[#470B37] truncate uppercase mt-0.5 tracking-widest opacity-60">{tx.fromCountry || 'RU'} ➔ {tx.toCountry || 'AF'}</p>
                </div>
