...
const StatusBadge = ({ status }: { status: TransactionStatus }) => {
  const styles: Record<string, string> = {
    ...
    completed: 'bg-[#E8DEF8] text-[#1D192B] border border-[#470B37]/10',
              ...
              {loading ? (
                ...
                filteredTransactions.map((tx: Transaction) => (
                  <tr key={tx.id} className="hover:bg-[#F3EDF7]/30 transition-all group cursor-pointer" onClick={() => navigate(`/queue/${tx.id}`)}>
                      ...
                      <div className="flex items-center gap-3">
                         ...
                         <div className="flex flex-col">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              <span className="text-[#470B37] text-[9px] font-black uppercase tracking-widest">{tx.isBulk ? 'Multi-Envoi' : 'Unique'}</span>
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col">
                        ...
                        <div className="flex flex-col mt-0.5">
                          {tx.clientPhone && (
                            <a href={`tel:${tx.clientPhone}`} className="text-[#470B37] text-[10px] font-bold hover:underline">{tx.clientPhone}</a>
                          )}
                          {tx.clientEmail && (
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col">
                        <div className="flex items-center gap-2 mb-1">
                           ...
                           <span className="text-[10px] font-black text-[#470B37] uppercase">{tx.currency}</span>
                        </div>
                    ...
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-3">
                         <div className="w-8 h-8 bg-[#F3EDF7] rounded-full flex items-center justify-center text-[#470B37] border border-[#E7E0EB]">
                            <Globe size={14} />
                         </div>
                         <div className="flex flex-col">
                    ...
                    <td className="px-8 py-6 text-right" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
                      <div className="flex justify-end gap-2">
                        <button 
                          ...
                          className="p-2.5 bg-white border border-[#E7E0EB] text-[#49454F] hover:bg-[#470B37] hover:text-white rounded-xl transition-all shadow-sm" 
                        >
                          <Eye size={18} />
                        </button>
