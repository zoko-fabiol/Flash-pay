...
export const PreferencesPage: React.FC = () => {
  ...
  const Toggle = ({ value, onChange }: { value: boolean; onChange: () => void }) => (
    <button
      onClick={onChange}
      className={`w-14 h-8 rounded-full transition-all flex items-center px-1 ${value ? 'bg-[#470B37]' : 'bg-slate-200'}`}
    >
      ...
      <div className="max-w-2xl mx-auto space-y-6 pb-10 px-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="rounded-[32px] bg-white border border-slate-100 shadow-sm overflow-hidden">
          <div className="px-6 pt-6 pb-3 flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-[#470B37]/10 flex items-center justify-center text-[#470B37]">
              <Type size={18} />
            </div>
                ...
                <button
                  ...
                  className={`flex-1 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                    preferences.fontSize === size 
                      ? 'bg-white text-[#470B37] shadow-md ring-1 ring-slate-100 scale-[1.02]' 
          ...
          <div className="divide-y divide-slate-50">
            ...
            <div className="flex items-center justify-between px-6 py-6">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-[#F9EEF5] flex items-center justify-center shrink-0">
                  <Mail size={18} className="text-[#470B37]" />
                </div>
                <div>
        ...
        <div className="flex gap-4 pt-4">
          <button
            ...
            disabled={saving}
            className="flex-[2] flex items-center justify-center gap-2 py-4 bg-[#470B37] text-white font-black rounded-2xl shadow-xl shadow-[#470B37]/20 hover:shadow-2xl hover:shadow-[#470B37]/30 transition disabled:opacity-50 active:scale-95"
          >
