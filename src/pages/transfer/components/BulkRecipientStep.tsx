  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4 duration-500">
      ...
      <div className="mb-10 bg-white rounded-[40px] border border-slate-100 shadow-[0_24px_60px_rgba(0,0,0,0.04)] overflow-hidden">
        <div className={`p-8 flex flex-col sm:flex-row items-center justify-between gap-6 transition-all ${isAmountMatching ? 'bg-emerald-50/30' : 'bg-[#470B37]/5'}`}>
          <div className="space-y-1 text-center sm:text-left">
            <p className={`text-[11px] font-black uppercase tracking-[0.2em] ${isAmountMatching ? 'text-emerald-600' : 'text-[#470B37]'}`}>
        ...
        <div className="h-2 bg-slate-100 w-full relative">
          <div 
            className={`h-full transition-all duration-700 ease-out ${isAmountMatching ? 'bg-emerald-500' : (totalDistributed > targetAmount ? 'bg-rose-500' : 'bg-[#470B37]')}`}
      ...
      <div className="space-y-6">
          ...
          <div key={i} className="bg-white rounded-[40px] border border-slate-100 shadow-[0_24px_60px_rgba(0,0,0,0.06)] p-6 sm:p-10 space-y-6 relative group overflow-hidden">
             <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-2xl bg-[#470B37]/10 flex items-center justify-center text-[#470B37] font-black">
                    {i + 1}
                  </div>
                ...
                <div className="flex items-center gap-2">
                  <button 
                    ...
                    className="p-3 rounded-2xl bg-slate-50 text-slate-500 hover:bg-[#470B37]/10 hover:text-[#470B37] transition-all"
              ...
              <div className="grid gap-6">
                <div>
                  <input
                    ...
                    className="w-full p-5 rounded-2xl border-2 border-slate-100 focus:border-[#470B37] outline-none text-slate-900 font-bold bg-slate-50 transition-all"
                  />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full p-5 rounded-2xl border-2 border-slate-100 focus:border-[#470B37] outline-none text-slate-900 font-bold bg-slate-50 transition-all pr-16"
                    />
        ...
        <button 
          ...
          className="w-full p-8 rounded-[40px] border-4 border-dashed border-slate-100 text-slate-300 font-black flex flex-col items-center justify-center gap-4 hover:border-[#470B37]/20 hover:text-[#470B37] hover:bg-[#470B37]/5 transition-all"
        >
          <div className="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center group-hover:bg-[#470B37]/10">
            <Plus size={32} />
          </div>
              ...
              {savedContacts.length === 0 ? (
                ...
                savedContacts.map((contact: any) => (
                  <button 
                    ...
                    className="w-full p-4 rounded-2xl border border-slate-100 hover:border-[#470B37] hover:bg-[#470B37]/5 flex items-center gap-4 transition-all text-left group"
                  >
                    <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 group-hover:bg-[#470B37]/10 group-hover:text-[#470B37] transition-colors font-bold">
                    ...
                    <ChevronRight size={18} className="text-slate-300 group-hover:text-[#470B37] transition-colors" />
                  </button>
                ))
              )}
            </div>
