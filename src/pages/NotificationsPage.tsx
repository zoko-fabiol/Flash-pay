    ...
    <Layout>
      <div className="max-w-3xl mx-auto pb-20 px-4 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="flex flex-col gap-6 pt-4">
          <button 
            onClick={() => navigate('/')} 
            className="w-fit inline-flex items-center gap-3 text-[11px] font-black uppercase tracking-[0.2em] text-[#470B37] hover:opacity-70 transition-all group px-2"
          >
            <div className="p-2 bg-[#470B37]/10 rounded-full group-hover:-translate-x-1 transition-transform">
              <ArrowLeft size={16} />
            </div>
            {t('back')}
          </button>
          ...
          <div className="flex items-end justify-between px-2">
            ...
            {unreadCount > 0 && (
              <div className="px-4 py-1.5 bg-[#470B37] text-white text-[10px] font-black uppercase tracking-widest rounded-full shadow-lg shadow-[#470B37]/30">
        ...
        <div className="sticky top-4 z-20 rounded-[28px] bg-white/80 backdrop-blur-xl border border-slate-200/50 shadow-xl shadow-slate-200/40 p-4 flex items-center justify-between gap-3 mx-1">
          <div className="flex items-center gap-2 pl-2">
            <Bell className={unreadCount > 0 ? "text-[#470B37] animate-bounce" : "text-slate-400"} size={20} />
        ...
        <div className="space-y-4">
          {isLoading ? (
            ...
            <div className="py-24 flex flex-col items-center text-center gap-6 bg-white rounded-[40px] border border-slate-100 shadow-sm px-10">
              <div className="w-24 h-24 rounded-[36px] bg-[#F9EEF5] flex items-center justify-center shadow-inner relative">
                <div className="absolute inset-0 bg-[#470B37]/5 rounded-[36px] animate-ping duration-[3000ms]"></div>
                <Bell size={40} className="text-[#470B37] relative z-10" />
              </div>
              ...
              <button 
                onClick={() => navigate('/')}
                className="mt-2 px-8 py-4 bg-[#470B37] text-white font-black uppercase text-xs tracking-widest rounded-full shadow-2xl shadow-[#470B37]/30 hover:scale-105 active:scale-95 transition-all"
              >
                {t('back_to_home')}
              </button>
            </div>
            ...
            <div className="grid gap-4">
              {notifications.map((item) => (
                <div 
                  ...
                  className={`group relative overflow-hidden rounded-[32px] border transition-all duration-300 hover:shadow-2xl hover:shadow-[#470B37]/10 hover:-translate-y-1 cursor-pointer ${
                    item.read 
                      ? 'bg-white border-slate-100 opacity-90' 
                      : 'bg-gradient-to-br from-white to-[#f9f7ff] border-[#eadfff] shadow-lg shadow-[#470B37]/5'
                  }`}
                >
                  <div className="p-6 flex gap-5">
                    ...
                    <div className="relative">
                      ...
                      {!item.read && (
                        <div className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-[#470B37] rounded-full border-2 border-white shadow-sm animate-pulse"></div>
                      )}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-3 mb-1">
                        <div className="space-y-1">
                          <p className={`font-black text-lg tracking-tight transition-colors ${item.read ? 'text-slate-800' : 'text-slate-900 group-hover:text-[#470B37]'}`}>
                            {item.title}
                          </p>
                          <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400">
                            ...
                            {!item.read && (
                              <>
                                ...
                                <span className="text-[#470B37]">{t('new')}</span>
                              </>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                          {!item.read && (
                            <button
                              ...
                              className="p-2.5 rounded-xl hover:bg-[#470B37]/10 text-[#470B37] transition-colors"
                              title={t('mark_read')}
                            >
                              <Check size={18} />
                            </button>
                      ...
                      <div className="mt-4 flex items-center text-[11px] font-black uppercase tracking-[0.15em] text-[#470B37] transition-all group-hover:translate-x-1 w-fit bg-[#470B37]/5 px-4 py-2 rounded-xl">
