...
const TransferJourneyPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubRates = onSnapshot(collection(db, 'exchange_rates'), (snapshot) => {
      const globalRates = snapshot.docs.map(doc => ({ id: doc.id, ...(doc.data() as any) }));
      onSnapshot(collection(db, 'custom_rates'), (snapshot2) => {
        const customRates = snapshot2.docs.map(doc => ({ id: doc.id, ...(doc.data() as any) }));
  ...
  const foundRate = rates.find((rate) => rate.from === 'RUB' && rate.to === (destinationCountry?.currency || transferData.currency || 'XAF'));
  const selectedRate = (foundRate as any)?.rate || (foundRate as any)?.rateFixed || 1.0;
  ...
  const renderSidebarSummary = () => (
    <aside className="hidden lg:flex lg:flex-col lg:gap-5">
      <div className="overflow-hidden rounded-[28px] border border-white/70 bg-gradient-to-br from-[#7b47de] via-[#6344B6] to-[#4a239c] p-7 text-white shadow-[0_28px_60px_rgba(98,54,204,0.22)]">
      ...
      <div className="rounded-[28px] border border-slate-100 bg-white p-6 shadow-sm">
        <p className="text-xs font-bold uppercase tracking-[0.24em] text-[#6344B6]">Aperçu</p>
          ...
          <section className="space-y-5">
            <div className="rounded-[28px] border border-white/70 bg-white p-5 shadow-[0_16px_50px_rgba(98,54,204,0.08)] sm:p-7">
              ...
              <div className="mt-5 text-center lg:text-left">
                <p className="text-xs font-bold uppercase tracking-[0.24em] text-[#6344B6]">Veuillez saisir les informations du destinataire</p>
              ...
              <div className="mt-6 space-y-4 rounded-[24px] border border-slate-100 bg-slate-50 p-4 sm:p-6">
                ...
                {selected?.operators?.length ? (
                  <div>
                    ...
                    <div className="rounded-2xl border border-[#eadfff] bg-[#F5E8FF] px-4 py-3 text-sm font-semibold text-[#6344B6] shadow-sm">
              ...
              <button
                ...
                className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#6344B6] px-6 py-4 text-base font-bold text-white shadow-[0_14px_30px_rgba(98,54,204,0.24)] transition-all disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500 lg:w-auto"
              >
                Continuer <ArrowRight size={18} />
              </button>

              <button className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full border border-[#6344B6]/20 bg-white px-6 py-4 text-sm font-semibold text-[#6344B6] lg:w-auto">
        ...
        <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          ...
          <section className="space-y-5">
            <div className="rounded-[28px] border border-white/70 bg-white p-5 shadow-[0_16px_50px_rgba(98,54,204,0.08)] sm:p-7">
              ...
              <div className="mt-5 text-center lg:text-left">
                <p className="text-xs font-bold uppercase tracking-[0.24em] text-[#6344B6]">Vérifier les détails</p>
              ...
              <div className="mt-6 overflow-hidden rounded-[28px] border border-slate-100 bg-slate-50 p-5 sm:p-7">
                <div className="flex items-center justify-center gap-4 text-4xl sm:text-5xl">
                  ...
                  <ArrowRight className="text-[#6344B6]" size={28} />
                  {recipientFlagImg ? (
                    <img src={recipientFlagImg} alt="Destination flag" className="w-8 h-8" />
                  ) : (
                    <Globe size={28} className="text-[#6344B6]" />
                  )}
                </div>
              ...
              <div className="mt-4 rounded-[24px] border border-slate-100 bg-[#F5E8FF] p-4 text-sm text-slate-700 shadow-sm">
                <div className="flex items-start gap-3">
                  <ShieldCheck className="mt-0.5 text-[#6344B6]" size={18} />
                  <div>
              ...
              <button onClick={() => nextStep()} className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#6344B6] px-6 py-4 text-base font-bold text-white shadow-[0_14px_30px_rgba(98,54,204,0.24)] lg:w-auto">
                Continuer <ArrowRight size={18} />
              </button>
            </div>
          </section>
        ...
        <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          ...
          <section className="space-y-5">
            <div className="rounded-[28px] border border-white/70 bg-white p-5 shadow-[0_16px_50px_rgba(98,54,204,0.08)] sm:p-7">
              ...
              <div className="mt-5 flex items-start justify-between gap-4 rounded-[24px] border border-[#6344B6]/15 bg-[#F5E8FF] p-5">
                <div>
                  <p className="text-sm font-semibold text-[#6344B6]">Effectuez le paiement sur l’un des numéros ci-dessous et envoyez la capture d’écran du reçu.</p>
                  <p className="mt-2 text-sm text-slate-600">Vous avez 20 minutes pour effectuer le paiement.</p>
                </div>
                <div className="rounded-full bg-white px-4 py-2 text-xl font-black text-[#6344B6] shadow-sm">{timerLabel}</div>
              </div>

              <div className="mt-6 space-y-4">
                {bankAccounts.map((bank, index) => (
                  <div key={`${bank.name}-${index}`} className="rounded-[24px] border border-slate-100 bg-slate-50 p-4 shadow-sm sm:p-5">
                    <div className="mb-3 inline-flex rounded-full bg-[#6344B6] px-3 py-1 text-xs font-bold text-white">{bank.title}</div>
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                      <div className="flex items-start gap-4">
                        <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-2xl bg-white shadow-sm">
                          {bank.logo ? <img src={bank.logo} alt={bank.name} className="h-full w-full object-contain p-2" /> : <span className="text-xl font-black text-[#6344B6]">{bank.name.charAt(0)}</span>}
                        </div>
                        <div>
                      ...
                      <button onClick={() => handleCopy(bank.number)} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-[#6344B6]/30 bg-white px-4 py-3 text-sm font-semibold text-[#6344B6] transition-all hover:bg-[#6344B6] hover:text-white">
                        <Copy size={16} /> Copier
                      </button>
                    </div>
                  </div>
              ...
              <div className="mt-4 rounded-[24px] border border-[#6344B6]/15 bg-[#F5E8FF] p-4 text-sm text-slate-700">
                <div className="flex items-start gap-3">
                  <MapPinned className="mt-0.5 text-[#6344B6]" size={18} />
              ...
              <div className="mt-6 rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex items-center gap-3 text-sm font-semibold text-slate-700">
                  <Upload size={16} className="text-[#6344B6]" /> Téléchargez la preuve de paiement
                </div>
                <p className="mt-1 text-sm text-slate-500">Formats acceptés : JPG, PNG - Taille max : 5 Mo</p>

                <label className="mt-4 block cursor-pointer rounded-[24px] border-2 border-dashed border-[#6344B6]/25 bg-[#faf8ff] p-6 text-center transition-all hover:border-[#6344B6]">
                  <input
                    type="file"
                    className="hidden"
                  ...
                  {proofFile ? (
                    ...
                    <>
                      <Paperclip className="mx-auto text-[#6344B6]" size={40} />
              ...
              <button
                ...
                className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#6344B6] px-6 py-4 text-base font-bold text-white shadow-[0_14px_30px_rgba(98,54,204,0.24)] disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500 lg:w-auto"
              >
                Continuer
              </button>
            </div>
      ...
      <Layout>
        <div className="mx-auto grid max-w-5xl gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <div className="hidden lg:block">
            <div className="rounded-[32px] bg-gradient-to-br from-[#efe7ff] to-white p-10 shadow-[0_18px_50px_rgba(98,54,204,0.12)]">
              <div className="mx-auto flex h-48 w-48 items-center justify-center rounded-full bg-white shadow-inner">
                <Send size={72} className="text-[#6344B6]" />
              </div>
            </div>
          </div>
          ...
          <div className="rounded-[32px] border border-white/70 bg-white p-6 text-center shadow-[0_16px_50px_rgba(98,54,204,0.08)] sm:p-10">
            <div className="mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-[#efe7ff] text-[#6344B6]">
            ...
            <div className="mt-6 rounded-[24px] bg-[#F5E8FF] p-4 text-left text-sm text-slate-700">
              <div className="flex items-start gap-3">
                <Clock3 className="mt-0.5 text-[#6344B6]" size={18} />
                <p>Votre paiement est en cours. Nous vous avertirons quand il sera terminé.</p>
              </div>
            </div>

            <button onClick={() => navigate(`/transactions/${createdTransactionId}`)} disabled={!createdTransactionId} className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#6344B6] px-6 py-4 text-base font-bold text-white disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500">
              Voir la transaction
            </button>

            <button onClick={() => { resetWizard(); navigate('/'); }} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-full border border-[#6344B6]/25 bg-white px-6 py-4 text-base font-bold text-[#6344B6]">
...
const SummaryRow = ({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) => (
  <div className="flex items-center justify-between gap-4 border-b border-slate-100 py-2 last:border-0 last:pb-0">
    ...
    <span className={`text-sm font-bold ${accent ? 'text-[#6344B6]' : 'text-slate-900'}`}>{value}</span>
  </div>
);
