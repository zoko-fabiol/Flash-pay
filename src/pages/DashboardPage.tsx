      ...
      <div className="space-y-6 pb-20">
        ...
        <section className="relative overflow-hidden rounded-[32px] bg-[#6750A4] p-8 text-white shadow-xl">
          ...
          <div className="relative z-10">
            <span className="inline-block px-3 py-1 rounded-full bg-white/10 text-[10px] font-black uppercase tracking-[0.2em] text-white/90 mb-6">Tableau de bord</span>
            <h1 className="text-4xl font-black leading-[1.1] sm:text-6xl tracking-tight">Bienvenue, <br/>{user?.nom?.split(' ')[0] || 'Utilisateur'}</h1>
            <p className="mt-6 max-w-[240px] text-base font-medium text-white/70 leading-relaxed">
              Transférez vos fonds instantanément avec Flash Pay.
            </p>
            <div className="mt-10">
              <button onClick={() => navigate('/transfer')} className="inline-flex items-center gap-3 rounded-full bg-[#EADDFF] px-8 py-5 text-sm font-black text-[#21005D] shadow-2xl hover:scale-105 transition-all">
                Démarrer un transfert <ArrowRight size={20} />
              </button>
            </div>
          </div>
        </section>
        ...
        {/* Referral Section */}
        {/* Referral Section - M3 Secondary Container */}
          ...
          <div className="flex justify-between items-start mb-8 relative z-10">
            <div className="max-w-[75%]">
              <h3 className="text-2xl font-black text-[#1D192B] leading-tight tracking-tight">
                Gagnez <span className="text-[#6750A4]">500 roubles</span> <br/>en parrainant vos amis.
              </h3>
            </div>
        ...
        {/* Exchange Rate Section - M3 Primary Container */}
        <section>
          <div className="bg-[#EADDFF] rounded-[32px] p-8 border border-[#D0BCFF]/30">
            <div className="flex items-center justify-between mb-6">
              <div className="px-4 py-2 bg-[#FEF7FF] rounded-xl flex items-center gap-2 text-[#21005D] shadow-sm font-black text-xs">
                <Globe size={16} /> TAUX ACTUEL
              </div>
              <TrendingUp className="text-[#6750A4]" size={24} />
            </div>
            <p className="text-[10px] font-black text-[#21005D]/60 uppercase tracking-[0.2em] mb-3">1 RUB VAUT ACTUELLEMENT</p>
        ...
        <section className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-[28px] border border-[#eadfff] bg-white p-5 shadow-[0_14px_40px_rgba(15,23,42,0.06)] sm:p-6">
            <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#6236CC]">Compte</p>
            <div className="mt-4 flex items-center justify-between gap-4">
              <div>
                <h3 className="text-xl font-black text-slate-900">Statut KYC</h3>
                <p className="mt-1 text-sm text-slate-500">Niveau de vérification</p>
              </div>
              <span className={`rounded-full px-3 py-1 text-sm font-bold ${
                ...
                {user?.kyc?.status || user?.statut_kyc || 'Standard'}
              </span>
            </div>
            <div className="mt-5 rounded-[24px] bg-slate-50 p-5">
              <p className="text-sm text-slate-500">Bonus disponible</p>
              <p className="mt-2 text-3xl font-black text-[#6236CC]">{user?.solde_bonus || 0} XAF</p>
              <p className="mt-1 text-sm text-slate-500">À utiliser sur vos transferts</p>
            </div>
          </div>
          ...
          <div className="rounded-[28px] border border-[#eadfff] bg-white p-5 shadow-[0_14px_40px_rgba(15,23,42,0.06)] sm:p-6">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#6236CC]">Activité récente</p>
                <h3 className="mt-2 text-2xl font-black text-slate-900">Transactions récentes</h3>
              </div>
            ...
            <div className="mt-4 space-y-3">
              {recentTransactions.length === 0 ? (
                <div className="rounded-[24px] border border-dashed border-slate-200 p-8 text-center text-slate-500">Aucune transaction pour le moment</div>
              ) : (
                recentTransactions.map((tx) => (
                  <button key={tx.id} onClick={() => navigate(`/transactions/${tx.id}`)} className="flex w-full items-center justify-between gap-4 rounded-[24px] border border-slate-100 bg-slate-50 px-4 py-4 text-left transition-all hover:border-[#6236CC]/30 hover:bg-white">
                    <div>
                      <p className="font-semibold text-slate-900">{tx.recipientName || 'Destinataire inconnu'}</p>
                      <p className="text-sm text-slate-500">{tx.createdAt?.toDate ? tx.createdAt.toDate().toLocaleDateString('fr-FR') : ''}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-black text-[#6236CC]">{tx.amount?.toLocaleString('fr-FR')} {tx.currency}</p>
                      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">{tx.status || 'pending'}</p>
                    </div>
                  </button>
                ))
              )}
